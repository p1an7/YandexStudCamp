...
class AStarPath:
    ...
    def a_star_search(self, start_x, start_y, end_x, end_y):
        ...
        while True:
            ...
            total_cost = min(record_open,
                             key=lambda x: record_open[x].cost + self.calc_heuristic(end_node, record_open[x]))
            cost_collection = record_open[total_cost]

            if show_animation:  # pragma: no cover
                plt.plot(self.calc_grid_position(cost_collection.x, self.x_min),
                         self.calc_grid_position(cost_collection.y, self.y_min), "xy")
            ...
            for i, _ in enumerate(self.path):
                node = self.Node(cost_collection.x + self.path[i][0], cost_collection.y + self.path[i][1],
                                 cost_collection.cost + self.path[i][2], total_cost)
...
def main():
    ...
    image = cv2.imread('C:\\Users\\Ivan\\PycharmProjects\\YandexStudCamp\\Dataset\\1-map.png')
...
class Algoritm_Deicstar:

    def __init__(self, start_char):
        self.start_vertex = start_char

    def dijkstra(self, graph):
        # Инициализация расстояний до всех вершин
        distances = {vertex: float('infinity') for vertex in graph}
        distances[self.start_vertex] = 0  # Расстояние до стартовой вершины равно 0

        # Используем приоритетную очередь для хранения вершин
        priority_queue = [(0, self.start_vertex)]  # (расстояние, вершина)
